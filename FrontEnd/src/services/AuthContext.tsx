...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      ...
      if (adminToken) {
        try {
          const response = await axios.get('https://mini-youtube-backend-hnra.onrender.com/api/admin/profile', {
      ...
      if (firebaseUser) {
        try {
          ...
          const response = await axios.post(
            'https://mini-youtube-backend-hnra.onrender.com/api/auth/login',
            {},
            {
        ...
        if (token) {
          try {
            const response = await axios.get('https://mini-youtube-backend-hnra.onrender.com/api/auth/me', {
              headers: { Authorization: `Bearer ${token}` },
            });
            setUser({
              id: response.data.id,
  ...
  const signup = async (email: string, password: string, name: string) => {
    ...
    try {
      const mongoCheck = await axios.post('https://mini-youtube-backend-hnra.onrender.com/api/manual/check-user', { email });
      ...
      const response = await axios.post('https://mini-youtube-backend-hnra.onrender.com/api/manual/signup', {
        userId,
        username: name,
        email,
        password,
  ...
  const login = async (email: string, password: string) => {
    ...
    setError(null);
    try {
      const response = await axios.post('https://mini-youtube-backend-hnra.onrender.com/api/manual/login', {
        email,
        password,
      });
