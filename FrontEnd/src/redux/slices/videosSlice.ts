...
export const fetchVideos = createAsyncThunk('videos/fetchVideos', async (_, { rejectWithValue }) => {
  try {
    ...
    const response = await fetch('https://mini-youtube-backend-hnra.onrender.com/api/public/videos', {
      headers: {
...
export const fetchVideosWithSearch = createAsyncThunk(
  ...
  async (searchQuery: string, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(`https://mini-youtube-backend-hnra.onrender.com/api/public/videos?search=${encodeURIComponent(searchQuery)}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
